...
RESULTS_FILE = PROJECT_ROOT / "evaluation" / "results" / "stage3_final_results.jsonl"

def main():
    print("=" * 80)
    print("VAANIRAG STAGE 3 FINAL RAG EVALUATION (Phase 1D)"
    )
    print("=" * 80)
    ...
    # 2. Load evaluation queries (30 queries)
    ...
    with open(RESULTS_FILE, "w", encoding="utf-8") as out:
        for idx, item in enumerate(queries, 1):
            ...
            # Simple back‑off handling for rate‑limit or other transient errors
            if not response.success and response.error_message and "429" in response.error_message:
                print("  Rate limit encountered – backing off for 20 seconds...")
                time.sleep(20.0)
            else:
                # Small pause to stay polite to the API
                time.sleep(4.5)

    print("\n" + "=" * 80)
    print(f"STAGE 3 FINAL RAG EVALUATION COMPLETE: {evaluated_count} queries processed.")
